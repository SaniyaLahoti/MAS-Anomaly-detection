"""
V2 LSTM SHAP Explainer using DeepExplainer

Provides interpretable explanations for V2 LSTM hierarchical model predictions.
"""
...
class V2LSTMHierarchicalSHAPExplainer:
    """
    SHAP explainer for V2 hierarchical LSTM models
    """
    
    def __init__(self, model_dir='../models/v2_lstm/'):
    ...
    def load_models(self):
        """
        Load V2 hierarchical LSTM models and artifacts
        """
        print(f"📂 Loading V2 LSTM models from {self.model_dir}")
        
        # Load models
        self.model_stage1 = load_model(f'{self.model_dir}/lstm_stage1_model.h5')
        self.model_stage2 = load_model(f'{self.model_dir}/lstm_stage2_model.h5')
        
        # Load scalers
        s1_mean = np.load(f'{self.model_dir}/scaler_stage1_mean.npy')
        s1_scale = np.load(f'{self.model_dir}/scaler_stage1_scale.npy')
        self.scaler_stage1 = StandardScaler()
        self.scaler_stage1.mean_ = s1_mean
        self.scaler_stage1.scale_ = s1_scale
        
        s2_mean = np.load(f'{self.model_dir}/scaler_stage2_mean.npy')
        s2_scale = np.load(f'{self.model_dir}/scaler_stage2_scale.npy')
        self.scaler_stage2 = StandardScaler()
        self.scaler_stage2.mean_ = s2_mean
        self.scaler_stage2.scale_ = s2_scale
        
        # Load encoders
        self.encoder_stage1_classes = np.load(f'{self.model_dir}/label_encoder_stage1.npy', allow_pickle=True)
        self.encoder_stage2_classes = np.load(f'{self.model_dir}/label_encoder_stage2.npy', allow_pickle=True)
        
        print("✅ V2 LSTM models loaded")
        print(f"   Stage 1 classes: {list(self.encoder_stage1_classes)}")
        print(f"   Stage 2 classes: {list(self.encoder_stage2_classes)}")
    ...
    def create_shap_explainers(self, background_data):
        """
        Create SHAP DeepExplainers for LSTM models
        """
        print("\n🔍 Creating V2 LSTM SHAP explainers...")
        
        # Prepare background data
        X_bg = self._engineer_features(background_data)
        X_bg_scaled = self.scaler_stage1.transform(X_bg)
        X_bg_reshaped = X_bg_scaled.reshape((X_bg_scaled.shape[0], 1, X_bg_scaled.shape[1]))
        
        # Create Stage 1 explainer
        self.explainer_stage1 = shap.DeepExplainer(self.model_stage1, X_bg_reshaped[:100])
        print("✅ Stage 1 SHAP explainer created")
        
        # Create Stage 2 explainer
        X_bg_scaled_s2 = self.scaler_stage2.transform(X_bg)
        X_bg_reshaped_s2 = X_bg_scaled_s2.reshape((X_bg_scaled_s2.shape[0], 1, X_bg_scaled_s2.shape[1]))
        self.explainer_stage2 = shap.DeepExplainer(self.model_stage2, X_bg_reshaped_s2[:100])
    ...
    def explain_prediction(self, input_df):
        ...
        X_reshaped_s1 = X_scaled_s1.reshape((X_scaled_s1.shape[0], 1, X_scaled_s1.shape[1]))
        
        # Stage 1 prediction
        pred_s1_proba = self.model_stage1.predict(X_reshaped_s1, verbose=0)[0]
        pred_s1 = np.argmax(pred_s1_proba)
        pred_s1_label = self.encoder_stage1_classes[pred_s1]
        
        # Stage 1 SHAP values
        shap_values_s1 = self.explainer_stage1.shap_values(X_reshaped_s1)
        
        # Extract SHAP values for predicted class
        if isinstance(shap_values_s1, list):
            shap_values_for_pred = shap_values_s1[pred_s1][0, 0, :]
        else:
            shap_values_for_pred = shap_values_s1[0, 0, :]
        ...
        result = {
            ...
            'all_probabilities': {cls: float(prob) for cls, prob in zip(self.encoder_stage1_classes, pred_s1_proba)},
        ...
        if pred_s1_label == 'DOS':
            ...
            X_reshaped_s2 = X_scaled_s2.reshape((X_scaled_s2.shape[0], 1, X_scaled_s2.shape[1]))
            
            pred_s2_proba = self.model_stage2.predict(X_reshaped_s2, verbose=0)[0]
            pred_s2 = np.argmax(pred_s2_proba)
            pred_s2_label = self.encoder_stage2_classes[pred_s2]
            
            # Stage 2 SHAP values
            shap_values_s2 = self.explainer_stage2.shap_values(X_reshaped_s2)
            
            if isinstance(shap_values_s2, list):
                shap_values_for_pred_s2 = shap_values_s2[pred_s2][0, 0, :]
            else:
                shap_values_for_pred_s2 = shap_values_s2[0, 0, :]
            ...
            result['stage2_probabilities'] = {cls: float(prob) for cls, prob in zip(self.encoder_stage2_classes, pred_s2_proba)}
